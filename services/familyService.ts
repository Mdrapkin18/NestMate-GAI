...
import { UserProfile, Invite, inviteSchema, Family } from '../types';

/**
 * Creates a new family for a user, making them the owner.
 * This function is now atomic, creating the family document and updating the user document
...
export const createFamily = async (userId: string): Promise<string> => {
    ...
    const newFamily: Omit<Family, 'id' | 'createdAt'> & { createdAt: any } = {
        ownerId: userId,
    ...
    // 2. Update the user's profile to link them to the new family
    ...
    await batch.commit();
    console.log(`[familyService] Successfully created family and updated user profile.`);
    return familyId;
};
