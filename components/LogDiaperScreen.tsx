...
type DiaperData = Omit<Diaper, 'id' | 'babyId' | 'familyId' | 'createdBy' | 'createdAt' | 'updatedAt' | 'startedAt' | 'endedAt'>;
...
export const LogDiaperScreen: React.FC<LogDiaperScreenProps> = ({ onBack, onSave }) => {
    const [type, setType] = useState<Diaper['type']>('pee');
    ...
    const isPoop = type === 'poop' || type === 'both';
    const isPee = type === 'pee' || type === 'both';

    const handleSave = () => {
        const saveData: DiaperData = { type };
            ...
            <div className="bg-light-surface dark:bg-dark-surface p-6 rounded-xl shadow-sm space-y-6">
                 <div>
                    ...
                    <div className="flex items-center space-x-2">
                        <SelectionButton label="Pee" isActive={type === 'pee'} onClick={() => setType('pee')} className="flex-1" />
                        <SelectionButton label="Poop" isActive={type === 'poop'} onClick={() => setType('poop')} className="flex-1" />
                        <SelectionButton label="Both" isActive={type === 'both'} onClick={() => setType('both')} className="flex-1" />
                    </div>
                </div>
