...
import { AnyEntry, Feed, Sleep, Pump, Diaper, Bath } from '../types';
...
// Form fieldsets for different entry types
const DiaperFormFields: React.FC<{formState: Diaper, setFormState: React.Dispatch<React.SetStateAction<AnyEntry>>}> = ({ formState, setFormState }) => {
    // FIX: Compare against 'diaperType' instead of 'type'. The 'type' is always 'diaper'.
    const isPoop = formState.diaperType === 'poop' || formState.diaperType === 'both';
    return (
        <>
            <InputField label="Time" id="startedAt" type="datetime-local" value={formatDateForInput(formState.startedAt)} 
                onChange={(e) => setFormState(prev => ({...prev, startedAt: new Date(e.target.value), endedAt: new Date(e.target.value)}))} />
            ...
            <div>
                ...
                <div className="flex items-center space-x-2">
                    {/* FIX: Update 'diaperType' property, not 'type'. Cast state to Diaper to avoid type errors. */}
                    <SelectionButton label="Pee" isActive={formState.diaperType === 'pee'} onClick={() => setFormState(p => ({...(p as Diaper), diaperType: 'pee'}))} className="flex-1" />
                    <SelectionButton label="Poop" isActive={formState.diaperType === 'poop'} onClick={() => setFormState(p => ({...(p as Diaper), diaperType: 'poop'}))} className="flex-1" />
                    <SelectionButton label="Both" isActive={formState.diaperType === 'both'} onClick={() => setFormState(p => ({...(p as Diaper), diaperType: 'both'}))} className="flex-1" />
                </div>
            </div>
            ...
            {isPoop && (
                <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <SelectionButton label="Runny" isActive={formState.consistency === 'runny'} onClick={() => setFormState(p => ({...(p as Diaper), consistency: 'runny'}))} />
                        <SelectionButton label="Mushy" isActive={formState.consistency === 'mushy'} onClick={() => setFormState(p => ({...(p as Diaper), consistency: 'mushy'}))} />
                        <SelectionButton label="Soft" isActive={formState.consistency === 'soft'} onClick={() => setFormState(p => ({...(p as Diaper), consistency: 'soft'}))} />
                        <SelectionButton label="Hard" isActive={formState.consistency === 'hard'} onClick={() => setFormState(p => ({...(p as Diaper), consistency: 'hard'}))} />
                        <SelectionButton label="Solid" isActive={formState.consistency === 'solid'} onClick={() => setFormState(p => ({...(p as Diaper), consistency: 'solid'}))} />
                    </div>
                </div>
            )}
        </>
...
const BathFormFields: React.FC<{formState: Bath, setFormState: React.Dispatch<React.SetStateAction<AnyEntry>>}> = ({ formState, setFormState }) => {
    return (
        <>
            <InputField label="Time" id="startedAt" type="datetime-local" value={formatDateForInput(formState.startedAt)} 
                onChange={(e) => setFormState(prev => ({...prev, startedAt: new Date(e.target.value), endedAt: new Date(e.target.value)}))} />
            ...
            <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <SelectionButton label="Sponge Bath" isActive={formState.bathType === 'sponge'} onClick={() => setFormState(p => ({...(p as Bath), bathType: 'sponge'}))} />
                    <SelectionButton label="Full Bath" isActive={formState.bathType === 'full'} onClick={() => setFormState(p => ({...(p as Bath), bathType: 'full'}))} />
                </div>
            </div>
        </>
    );
...
export const EditEntryScreen: React.FC<EditEntryScreenProps> = ({ entry, onBack, onSave, onDelete }) => {
    ...
    const title = useMemo(() => {
        if ('kind' in entry) return entry.kind === 'pump' ? 'Edit Pump' : 'Edit Feed';
        if ('category' in entry) return 'Edit Sleep';
        if ('diaperType' in entry) return 'Edit Diaper';
    ...
    const renderFormFields = () => {
        ...
        if (formState.type === 'diaper') { // Diaper
            return <DiaperFormFields formState={formState as Diaper} setFormState={setFormState} />;
        }
        if (formState.type === 'bath') { // Bath
            return <BathFormFields formState={formState as Bath} setFormState={setFormState} />;
        }
